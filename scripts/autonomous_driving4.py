...
class HybridFollowerSmooth:
    def __init__(self):
        ...
        self.kp_wall = 0.3
        self.wall_speed = 3.0   # base wall speed
        ...
        self.max_steering_delta = 0.08
        self.steering_ema_alpha = 0.7
        ...
        self.boost_rate = 0.05     # increment per callback when smooth
        self.max_boost = 1.5       # extra speed ceiling
        self.decay_rate = 0.2      # bleed-off per callback when not smooth
    ...
    def scan_callback(self, scan: LaserScan):
        ...
        if self.state == 'WALL':
            ...
            # reset boost while gap-following
            self.current_boost = 0.0
            ...
            # Optionally blend in boost during transition (more stable feel)
    ...
    def compute_wall_steer(self, scan, ranges):
        ...
        valid = sector[(sector > 0.0) & (sector < scan.range_max)]
        measured = float(np.median(valid)) if len(valid) > 0 else scan.range_max
