...
import sys, termios, tty, threading, select  # for keyboard control

class HybridFollowerSmooth(Node):
    def __init__(self):
        super().__init__('hybrid_follower_smooth')
        ...
        self.max_steering_delta = 0.28
        self.steering_ema_alpha = 0.10  
        ...
        self.get_logger().info("HybridFollowerSmooth ready. Press X to pause/resume, Q to quit.")
            ...
            while not self.shutdown_requested:
                ...
                if dr:
                    ...
                    if ch.lower() == 'x':
                        ...
                        if self.manual_stop:
                            self.get_logger().warn("!!! EMERGENCY STOP ENGAGED (X pressed) !!!")
                            self.publish_drive(0.0, 0.0)
                        else:
                            self.get_logger().info("Resuming autonomous driving.")
                    elif ch.lower() == 'q':
                        self.get_logger().warn("Shutdown requested (Q pressed).")
    ...
    def scan_callback(self, scan: LaserScan):
        if self.manual_stop or self.shutdown_requested:
            self.publish_drive(0.0, 0.0)
            return
        ...
        if self.state == 'WALL':
            if forward_dist < self.enter_gap_thresh:
                self.get_logger().info("[Switch] WALL -> GAP (obstacle ahead)")
                self.state = 'GAP'
            else:
                steer = self.compute_wall_steer(scan, ranges)
                speed = self.wall_speed

                if abs(steer) < 0.25 and forward_dist > self.forward_emergency_dist:
            ...
            if (forward_dist > self.exit_gap_thresh) and (wall_conf >= self.wall_confidence_threshold):
                self.get_logger().info("[Switch] GAP -> TRANSITION (conditions met to re-acquire wall)")
            ...
            if self.transition_counter >= self.transition_steps:
                self.get_logger().info("[Switch] TRANSITION -> WALL (re-acquired)")
                self.state = 'WALL'
    ...
    def publish_drive(self, steering, speed):
        ...
        drive_msg.drive.speed = float(np.clip(speed, 0.0, self.wall_speed + self.max_boost))
        drive_msg.drive.steering_angle = float(np.clip(steering, -self.max_steering, self.max_steering))
