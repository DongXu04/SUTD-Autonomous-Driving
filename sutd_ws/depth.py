...
class HybridCarController(Node):
    ...
    def handle_manual_mode(self, msg: Joy):
        ...
        throttle = -msg.axes[self.axis_throttle]
        ...
        steering_angle = np.clip(steering * self.manual_steering_scale,
                                 -self.manual_steering_scale, self.manual_steering_scale)
    ...
    def scan_callback(self, scan: LaserScan):
        ...
        for (s, e) in free_segments:
            ...
            valid_seg = seg_ranges[np.isfinite(seg_ranges)]
            if len(valid_seg) == 0:
                depth = scan.range_max
            else:
                depth = np.nanmean(valid_seg)

            score = (width_angle * bottleneck) * (depth * 0.75)

            if score > best_score:
                best_score = score
                best_segment = (s, e, bottleneck, width_angle, depth)
        ...
        if best_segment is None:
            ...
            s, e, bottleneck, width_angle, depth = best_segment
            target_idx = (s + e) // 2
