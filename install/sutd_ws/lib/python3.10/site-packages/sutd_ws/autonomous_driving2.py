...
class DisparityBiggestGapFollower(Node):
    def __init__(self):
        ...
        self.rb_button_index = 5  # RB button on Logitech F710 (in X mode)

        self.get_logger().info("DisparityBiggestGapFollower ready. Hold RB to enable motion.")

    def joy_callback(self, msg: Joy):
        """Reads RB button state for safety interlock."""
        if len(msg.buttons) > self.rb_button_index:
            self.safety_enabled = msg.buttons[self.rb_button_index] == 1
        else:
            self.safety_enabled = False

    def scan_callback(self, scan: LaserScan):
        """Main LIDAR-based control loop."""
        drive_msg = AckermannDriveStamped()

        # If RB not held, stop everything
